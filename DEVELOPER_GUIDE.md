# AfterPulse — Developer Guide

AfterPulse is an AI-powered post-purchase and checkout upsell app for Shopify, built on the React Router TypeScript template with Prisma + SQLite for local dev.
...
> **Windows note:** these instructions assume PowerShell 7+. Commands that set environment variables use PowerShell syntax (`$env:PORT=...`). If you are on macOS/Linux, use the POSIX form already present in `package.json` (`PORT=6300 shopify app dev ...`).
...
```powershell
git clone <repo-url> after-pulse
cd after-pulse
npm install
```
...
The committed `shopify.app.toml` is bound to **client_id `5ef7968e857f71492169fffab4e38a81`**, which belongs to the original Partner organization. You cannot reuse it — create your own:

1. Go to https://partners.shopify.com → **Apps** → **Create app** → **Create app manually**.
2. Give it any name (e.g. `AfterPulse Dev — <your-name>`).
...
This creates `prisma/dev.sqlite`. The Shopify session store lives here, along with the AfterPulse domain tables (shops, offers, AI rules, analytics).

---

## 6. Start your tunnel

You need a public HTTPS URL that forwards to **port 6300** on your machine.
...
```powershell
$env:PORT=6300
shopify app dev --tunnel-url=https://<your-tunnel-host>:6300
```

Replace `<your-tunnel-host>` with the hostname from step 6 (no scheme/port duplication — the `:6300` after the host tells Shopify which port to forward to).
...
- **Dashboard** (`/app`) — date-range metrics from the local DB
- **Offers** (`/app/offers`) — create/edit/delete upsell offers (`app.offers_.new.tsx`, `app.offers_.$id.tsx`)
- **AI Rules** (`/app/ai-rules`) — configure smart-rule generation
- **Analytics** (`/app/analytics`) — performance per offer
- **Settings** (`/app/settings`) — brand colors, AI toggle, API keys
- **Order webhook** (`/webhooks/orders/create`) — place a test order in the dev store to trigger it
...
- **`PORT` env var ignored on Windows** → use `$env:PORT=6300` *before* `shopify app dev`, not inline. Inline `PORT=6300 shopify app dev` is a bash-only syntax.
