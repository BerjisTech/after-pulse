...
/**
 * Format a percentage value with a sign indicator.
 */
...
/**
 * Calculate AOV lift percentage.
 */
export function calcAovLift(
  baseAov: number,
  upsellRevenue: number,
  totalOrders: number,
): number {
  if (baseAov === 0 || totalOrders === 0) return 0;
  const liftPerOrder = upsellRevenue / totalOrders;
  return (liftPerOrder / baseAov) * 100;
}
