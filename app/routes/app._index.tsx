...
import type { DashboardMetrics } from "../lib/offers/types";
    ...
    <s-page heading="Dashboard">
          ...
          {metrics.revenueByDay.length > 0 ? (
            ...
            <div className={styles["empty-state"]}>
              ...
              <div className={styles["empty-state__description"]}>
                Revenue data will appear here once your offers start generating conversions.
              </div>
            </div>
          )}
        </div>
      ...
      <s-section heading="Top Performing Offers">
        {metrics.topOffers.length > 0 ? (
          <div className={styles["top-offers"]}>
            <table className={styles["top-offers__table"]}>
              <thead>
                <tr>
                  <th>Offer</th>
                  <th>Impressions</th>
              ...
              <tbody>
                {metrics.topOffers.map((offer) => (
                  <tr key={offer.id}>
                    <td>
                      <Link to={`/app/offers/${offer.id}`} className={styles["offers-table__name"]}>
                        {offer.name}
                      </Link>
                    </td>
          ...
          <div className={styles["empty-state"]}>
            <div className={styles["empty-state__icon"]}>🎯</div>
            <div className={styles["empty-state__title"]}>No offers yet</div>
            <div className={styles["empty-state__description"]}>
              Create your first upsell offer to start tracking performance.
            </div>
      ...
      <s-section slot="aside" heading="Quick Actions">
        <div style={{ display: "grid", gap: "0.75rem" }}>
          <Link to="/app/offers/new" className={styles["quick-action-card"]}>
            <div className={styles["quick-action-card__icon"]}>🚀</div>
            <div className={styles["quick-action-card__title"]}>Create Offer</div>
            <div className={styles["quick-action-card__description"]}>
              Set up a new upsell or bundle offer
            </div>
          </Link>
          <Link to="/app/ai-rules" className={styles["quick-action-card"]}>
            <div className={styles["quick-action-card__icon"]}>🤖</div>
            <div className={styles["quick-action-card__title"]}>AI Smart Rules</div>
            <div className={styles["quick-action-card__description"]}>
              Let AI generate personalized offers
            </div>
          </Link>
      ...
      <s-section slot="aside" heading="Getting Started">
        <s-unordered-list>
          <s-list-item>
            <s-link href="/app/offers/new">Create your first upsell offer</s-link>
          </s-list-item>
          <s-list-item>
            <s-link href="/app/settings">Configure your AI settings</s-link>
          </s-list-item>
          <s-list-item>
            <s-link href="/app/ai-rules">Generate offers with AI</s-link>
          </s-list-item>
        </s-unordered-list>
      </s-section>
    </s-page>
